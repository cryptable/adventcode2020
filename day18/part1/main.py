# Vocabularia
# expression : <statement> (<operand> <statement>)*
...
#    print(idx)
    if tokens[idx].token == 'number':
        ...
        (idx, parenthese_node) = processExpression(tokens, idx)
        if tokens[idx].token != ')':
            raise Exception("Unbalanced braces")
        idx += 1
...
def pass2_parser(tokens):
    idx = 0
    (idx, val) = processExpression(tokens, idx)
    if idx != len(tokens):
